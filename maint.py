...
def get_message_store():
    """returns message database store"""
    if USE_MYSQL:
...
async def process_update(event):
    """process incoming mastodon update (new posts in home feed)"""
    try:
...
async def process_notification(event):
    """Process incoming mastodon notifications"""
    try:
...
def process_xmpp_message_home(message):
    ...
    if re.match(r'h(elp)?$', body, re.I | re.MULTILINE):
        ...
        msg = XMPP.make_message(
            message['jid'],
            text_constants.HELP_MESSAGE.format(HOST),
            mtype='chat',
            mfrom='home@' + HOST)
        msg.send()
        ...
        t = threading.Thread(
            target=mastodon_get_thread_process,
            args=(message['jid'], mes_x),
            name='mastodon_getthread_' + message['jid'])
        t.start()
        ...
        while len(strings) > 0:
            ...
            if q and q.group(1):
                ...
                strings.append(line)  # return a string back
        ...
        msg = XMPP.make_message(
            message['jid'],
            'Unknown command\n' +
            f'If you want to write new post then please send your message to contact new@{0}'.format(HOST),
            mtype='chat',
            mfrom='home@' + HOST)
        msg.send()
...
def process_xmpp_message_new(message):
    ...
    body = message['body']
    t = threading.Thread(
        target=mastodon_post_status_process,
        args=(message['jid'], 0, body, 'public'),
...
def process_xmpp_message_thread(message):
    ...
    if body == '.':
        ...
        toot = message_store.get_message_by_id(user['mid'], mid)
        if not toot:
        ...
        answer = body
        last_message = message_store.get_message_by_id(user['mid'], message_id)
            ...
            if re.match(r'^(?:>|»)', body, re.I | re.MULTILINE):  # quotation
                ...
                while len(strings) > 0:
                    ...
                    if q and q.group(1):
                        ...
                        strings.insert(0, line)  # return a string back
                        break  # end of quotation
                print('Original post was:')
                original_post = original_post.rstrip()
                print('"' + original_post + '"')
                ...
                if len(answer) > 2:
                    ...
                    if answer.upper() == 'W':
                        ...
                        thread = threading.Thread(
                            target=mastodon_reblog_fav_status_process,
                            args=('f', message['jid'], message_id),
                            name='mastodon_favourite_' + message['jid'])
                        thread.start()
                        # MTHREADS.append(t)
                    else:
...
def process_xmpp_config(message):
    ...
    if not user:
        ...
        if body.startswith('server '):
            ...
            t = threading.Thread(
                target=mastodon_register_process,
                args=(server, message['jid']),
                name='mastodon_' + server)
            t.start()
            # MTHREADS.append(t)
        else:
            msg = XMPP.make_message(
        ...
        if user['status'] == 'registration':
            ...
            t = threading.Thread(
                target=mastodon_register_finish_process,
                args=(message['jid'], code),
                name='mastodon_' + message['jid'])
            t.start()
            # MTHREADS.append(t)
        else:
            body = body.lower()
            ...
            if body.startswith('server '):
                ...
                try:
                    ...
                    print(accts)
                    line = users_db.getAutoboostByJid(message['jid'])
                    print(line)
                    if not accts:
                        msg = XMPP.make_message(
                            message['jid'],
...
def process_xmpp_reaction_home(message):
    ...
    if toot:
        ...
        elif message['reaction'] == 'thread':
            threading.Thread(
                target=mastodon_get_thread_process,
                args=(message['jid'], message['id']),
                name='mastodon_get_thread_' + message['jid']).start()
...
def mastodon_processor(login, v):
    ...
    mastodonUser = MastodonUser(login, v.get('token'))
    mastodonUser.add_jids(v.get('jids'))
...
if __name__ == '__main__':
    ...
    XMPP.add_event_handler('session_start', lambda _: XMPP.connected_event.set())
    ...
    for k, v in USER_LIST.items():
        t = threading.Thread(
            target=mastodon_processor,
            args=(k, v),
            name='mastodon_listener_' + str(k))
        t.start()
        MTHREADS.append(t)
    XMPP.connect()
    print("after loop")
